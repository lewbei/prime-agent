...
export class ReplKernelManager {
	...
	private kernelLaunch(python: string): { executable: string; args: string[] } {
		if (!this.processSandbox) return { executable: python, args: ["-m", "rlm.repl"] };
		...
		const sandboxPrefix = this.processSandbox.argv.slice(0, separator + 1);
		return {
			executable: sandboxPrefix[0]!,
			args: [...sandboxPrefix.slice(1), python, "-m", "rlm.repl"],
		};
	}
	...
	private async doStart(startOptions: KernelStartOptions): Promise<void> {
		...
		const launch = this.kernelLaunch(python);
	...
	/** Backward-compatible alias retained for fork integrations that owned KernelManager directly. */
	async dispose(): Promise<void> {
		await this.shutdown({ snapshot: true, drainHostRequests: true });
	}
