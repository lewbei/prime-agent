...
export class AgentSession {
	...
	private _requireAvoRuntime(): AvoSessionRuntime {
		...
		if (!this._avoRuntime) {
			this._avoRuntime = new AvoSessionRuntime(
				this.sessionManager.getSessionArtifactDir(),
				this.sessionManager.getSessionId(),
				undefined,
				this._cwd,
	...
	private _readRlmLastAssistantText(subagent: RlmSubagentRegistryEntry): string | undefined {
		const liveSession =
			...
			this._rlmChildSessions.get(subagent.rlm_child_id)?.session;
	...
	async disposeAsync(options?: { kernelSnapshot?: boolean }): Promise<void> {
		...
		this._disposeAsyncPromise = this._disposeWithinDeadline(kernelSnapshot);
		return this._disposeAsyncPromise;
	}

	private async _disposeWithinDeadline(kernelSnapshot: boolean): Promise<void> {
		...
		try {
			await Promise.race([this._disposeGracefully(kernelSnapshot), deadline]);
		} finally {
			if (timeout) clearTimeout(timeout);
		}
	}

	private async _disposeGracefully(kernelSnapshot: boolean): Promise<void> {
		if (this._disposed) {
			return this._disposeCallbacksPromise;
		}
